...
\label{subsec:requirements-specs}
...
\begin{basedescript}{
	...
	\item[\textit{UC13: }Export Brainstorming Finding] Als Participant will ich ein abgeschlossenes BrainstormingFinding exportieren können. 
\end{basedescript}
\vspace{1cm}
...
\begin{center}
	\begin{longtable}{| c | p{7cm} |}
		...
		\textit{Primary Actor} & Participant\\
		\hline
		\textit{Stakeholders \& Interests} & Ein Participant wünscht als Teil seiner Brainwave ein Bild zu teilen. \\
		\hline
...
\begin{center}
	\begin{longtable}{| c | p{7cm} |}
		\hline
		\multicolumn{2}{|c|}{\textbf{Use-Case 8f: Record Video}}\\
		\hline\hline
		\textit{Primary Actor} & Participant\\
		\hline
...
\paragraph{Sequenzdiagramm}\label{sequence-diagram}~\\
...
\begin{basedescript}{
	...
	In einem nächsten Schritt wird wie bis anhin (siehe 12.1 in Abbildung \ref{fig:Seq-Methode635}) das gesamte Brainsheet, welches nun auch die ID in der entsprechenden SketchIdea gespeichert hat, an die API geschickt.
\end{basedescript}
...
\begin{basedescript}{
	...
	Auch hier wird nun wieder, wie bis anhin (siehe 12.1 in Abbildung \ref{fig:Seq-Methode635}), das gesamte Brainsheet, welches nun auch das Pattern in der entsprechenden PatternIdea gespeichert hat, an die API geschickt.
\end{basedescript}
...
\label{subsub:nfr}
Wie schon in unserer Studienarbeit halten wir uns auch hier wieder an die Standards ISO 9126\cite{ISO9126} bzw. dessen Nachfolger ISO 25010\cite{ISO9126_ISO25010}, welcher Gegensatz zum Standard ISO 9126 noch ausführlicher ist. Beide ISO-Normen liefern daher eine gute Checkliste für jegliche Art von nicht-funktionalen Anforderungen.
